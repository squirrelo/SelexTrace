...
from cogent.app.vienna_package import get_secondary_structure
...
def fold_groups(seqs, struct, hold, numkept=None):
    ...
    try:
        if numkept != None and len(seqs) > numkept:
            seqs = seqs[:numkept]
        aln, currstruct = run_locarnap(seqs, foldless=True)
        if currstruct == "":
            currstruct = struct
...
def run_locarnap_for_infernal(currgroup, groupfasta, basefolder):
    '''Function for multithreading
...
if __name__ == "__main__":
    ...
    if not exists(otufolder + "cluster_structs.fasta"):
        ...
        print "Running locarna-p over "+ str(len(clusters)) +" clusters"
        secs = time()
    ...
    if not skipiter:
        ...
        for struct in structgroups:
            if "(" not in struct:
                count += 1
        print count, "of", len(structgroups), " structureless"
        #try to match sequences in unfoldable clusters to other clusters that were folded
        topop = []
        orphanseqs = open(otufolder + "orphanseqs.txt", 'a')
        orphanseqs.write("iteration " + str(iteration) + "\n")
        for currstruct in structgroups:
            if currstruct.count("(") < 5:
                topop.append(currstruct)
                for seq in structgroups[currstruct]:
                    found = False
                    sequence, structure, energy = get_secondary_structure(seq[1])
                    for gstruct in structgroups:
                        if score_rnaforester(structure, gstruct) >= foresterscore:
    ...
    for group in walk(otufolder + "fasta_groups").next()[2]:
        groupnum = int(group.split("_")[-1].split(".")[0])
        pool.apply_async(func=run_locarnap_for_infernal, args=(groupnum, otufolder+"fasta_groups/" + group, otufolder))
    pool.close()
    pool.join()
    ...
    for group in walk(otufolder).next()[1]:
        ...
        if seqs > args.minseqs and not skip:
        ...
        #skip group if less than 100 sequences
            ...
            print "Group has less than " + str(args.minseqs) + "sequences, skipping infernal run"
