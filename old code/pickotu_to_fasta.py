def pickotu_to_fasta(fasta, otulist, folderout, minseqs=0):
    ...
    seqs = {}
    for name, seq in MinimalFastaParser(fastain):  # Clean seq names so only using the identifier before first space
    ...
    for line in otusin:
        ...
        if len(splitnames) - 1 > minseqs:
            summary.append((splitnames[0], len(splitnames) - 1))
